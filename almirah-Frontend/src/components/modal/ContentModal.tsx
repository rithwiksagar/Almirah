  ...
  return (
    <>
      {modal ? (
        <div
          onClick={toggleModal}
          className="w-screen h-screen pb-20 bg-black/70 fixed top-0 left-0 flex justify-center items-center"
        >
          <div
            className="bg-lightPink p-6 rounded-md space-y-6"
            onClick={(e) => {
              e.stopPropagation();
            }}
          >
            <div className="flex justify-between">
              <h2 className="p-1 font-clashdisplay font-semibold text-xl">
                Add Content
              </h2>
              <RxCross2
                onClick={toggleModal}
                className=" flex ml-auto text-xl  hover:bg-pink-200 w-fit h-fit p-1 rounded-full hover: cursor-pointer transition-all"
              />
            </div>
